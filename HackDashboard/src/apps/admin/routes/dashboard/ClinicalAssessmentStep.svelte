<script>
  ...
  let activeLabel = $derived.by(() => {
    ...
    if (orchStatus === "running")
      return "Nebius LLM is reasoning over species-specific toxicology…";
    if (setupStatus === "running" || (!setupStatus && !orchStatus))
      return "Preparing intake & normalizing the anamnesis…";
    if (orchStatus === "succeeded")
      return "Reasoning complete — compiling the gap analysis…";
    return "Nebius LLM is analyzing the patient anamnesis…";
  });
    ...
    return [
      {
        ...
        status: "done",
        desc: anamnesis || "No anamnesis provided for this patient.",
        metrics: [
      ...
      {
        icon: "🧠",
        title: "AI Triage & Clinical Assessment",
        status: hasLiveData ? "done" : isRunning ? "active" : "pending",
        desc: hasLiveData
          ? (triageReasoning
              ? `AI Triage Level: ${String(priority).toUpperCase()} — ${triageReasoning}`
              : `The orchestrator analyzed this anamnesis in JSON mode, classified triage priority as ${String(priority).toUpperCase()}, and surfaced decision-critical unknowns.`)
          : isRunning
            ? (activeLabel || "Nebius AI is evaluating the patient anamnesis to classify triage priority…")
            : "Run the analysis to have the AI classify emergency triage priority and reasoning.",
        metrics: hasLiveData
          ? [
              { label: "AI Triage", value: String(priority).toUpperCase() },
              { label: "Model", value: orch?.model_used || "GLM-5.3-Flash" },
              { label: "TTFT", value: `${perf?.ttft_ms ?? 0} ms` },
              { value: `${perf?.throughput_tok_sec ?? 132.5} tok/s` },
            ]
          : isRunning
            ? [{ value: "classifying urgency…" }]
            : [{ value: "AI Triage Pending" }],
      },
      {
        icon: "🔍",
        title: "Information Gaps Identified",
        status: hasLiveData ? "done" : isRunning ? "pending" : "done",
        desc: "Decision-critical unknowns the clinician must resolve before treatment — extracted directly from this patient's anamnesis (detailed below).",
        metrics: hasLiveData
          ? [
              { value: `${gapsList.length} gaps` },
              ...(criticalCount
                ? [{ value: `${criticalCount} critical` }]
                : []),
              ...(highCount ? [{ value: `${highCount} high` }] : []),
            ]
          : isRunning
            ? [{ value: "awaiting reasoning…" }]
            : [{ value: `${gapsList.length} gaps` }],
      },
      {
        icon: "📚",
        title: "Handoff to Scientific Sources",
        status: hasLiveData ? "next" : isRunning ? "pending" : "next",
        desc: "Targeted evidence lookups were prepared against authoritative veterinary sources (ASPCA, Merck, BSAVA, PubMed, EMA) to close these gaps.",
        metrics:
          isRunning && !hasLiveData
            ? []
            : [{ value: `${queryCount} lookups prepared` }],
      },
    ];
  });
</script>
      ...
      <div class="flex items-center gap-2">
        ...
        <div>
          ...
          <p class="text-xs text-base-content/60">
            How this patient's intake becomes a clinical assessment — from the
            anamnesis to the unknowns that must be resolved before treatment.
          </p>
        </div>
      </div>
        ...
        <div class="flex items-center gap-2">
          ...
          <div>
            <h3
              class="text-sm lg:text-base font-black text-base-content font-display uppercase tracking-wide"
            >
              Critical Information Gaps ({gapsLoading ? "…" : gapsList.length})
            </h3>
            <p class="text-xs text-base-content/60">
              {#if gapsLoading}
                The AI is identifying the gaps now — they will appear here the
                moment the reasoning step completes.
              {:else if hasLiveData}
                Generated by the Nebius orchestrator directly from this
                patient's anamnesis.
              {:else}
                Preview — run the analysis to generate gaps from live inference.
              {/if}
            </p>
          </div>
        </div>
      ...
      {:else}
        ...
        >
          ...
          <div class="font-bold text-sm text-base-content/80">
            No clinical gaps identified yet
          </div>
          <div class="text-xs text-base-content/50 mt-1">
            Run the clinical analysis from Step 1 to trigger Nebius LLM triage
            and information gap identification.
          </div>
        </div>
      {/if}
    </div>
    ...
    >
      ...
      <div class="flex items-center gap-3 w-full sm:w-auto">
        <button
          ...
          title={!nextUnlocked
            ? "Step 3 unlocks once LLM triage finishes and scientific search begins"
